    ...
    def loss(self,
        ...
        if cls_reg_targets is None:
            for featmap in cls_scores:
                assert isinstance(featmap, torch.Tensor), cls_scores
            for featmap in bbox_preds:
                assert isinstance(featmap, torch.Tensor), bbox_preds

            losses_cls = [featmap.sum() * 0.0 for featmap in cls_scores]
            losses_bbox = [featmap.sum() * 0.0 for featmap in bbox_preds]
