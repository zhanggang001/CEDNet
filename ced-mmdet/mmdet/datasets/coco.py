...
class CocoDatasetCP(CocoDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        # source image data
        while True:
            src_idx = np.random.randint(len(self))
            if src_idx != idx:
                break
        img_info = self.data_infos[src_idx]
        ann_info = self.get_ann_info(src_idx)
        src_results = dict(img_info=img_info, ann_info=ann_info)
        self.pre_pipeline(src_results)
        results['src_results'] = src_results
