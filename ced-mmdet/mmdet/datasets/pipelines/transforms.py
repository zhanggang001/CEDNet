...
class CopyPaste(object):
    ...
    def __init__(self, sub_pipeline, p=0.25, iou_thr=0.2, ext_pixels=16, max_paste_objects=7):
        self.p = p
        self.iou_thr = iou_thr
        self.ext_pixels = ext_pixels
        self.sub_pipeline = AugCompose(sub_pipeline)
        self.max_paste_objects = max_paste_objects

    def __call__(self, results):
        results = self.sub_pipeline(results)
        if results is None:
            return results

        if random.random() < self.p:
            src_results = results['src_results']
            src_results = self.sub_pipeline(src_results)
            if src_results is None:
                return results

            src_img, src_boxes, src_labels = [src_results[k] for k in ('img', 'gt_bboxes', 'gt_labels')]
            assert results['img'].shape == src_results['img'].shape

            if len(src_boxes) * len(results['gt_bboxes']) == 0:
                assert f"{len(src_boxes)}, {results['gt_bboxes']}"
                return results

            # filter src_boxes with iou > thr.
            max_ious = self.computer_src_iou(src_boxes, results['gt_bboxes'])
            keep_indices = max_ious < self.iou_thr
            src_boxes = src_boxes[keep_indices]
            src_labels = src_labels[keep_indices]

            if len(src_boxes) > 0:
                # randomly choose a subset
                num_paste_objects = min(len(src_boxes), self.max_paste_objects)
                keep_indices = np.random.choice(np.arange(len(src_boxes)), num_paste_objects)
                src_boxes = src_boxes[keep_indices]
                src_labels = src_labels[keep_indices]

                # add random context
                exts = np.random.randint(self.ext_pixels, size=(len(src_boxes), 4))
                patch_boxes = copy.deepcopy(src_boxes).astype(np.int32)
                patch_boxes[:, 0] = (patch_boxes[:, 0] - exts[:, 0]).clip(min=0)
                patch_boxes[:, 1] = (patch_boxes[:, 1] - exts[:, 1]).clip(min=0)
                patch_boxes[:, 2] = (patch_boxes[:, 2] + exts[:, 2]).clip(max=src_img.shape[0] - 1)
                patch_boxes[:, 3] = (patch_boxes[:, 3] + exts[:, 3]).clip(max=src_img.shape[1] - 1)

                binary_mask = np.zeros_like(results['img'])
                for x1, y1, x2, y2 in patch_boxes[:, :4]:
                    binary_mask[y1: y2, x1: x2, :] = 1

                tgt_img = results['img']
                results['img'] = (tgt_img * (1 - binary_mask) + src_img * binary_mask).astype(tgt_img.dtype)
                results['gt_bboxes'] = np.concatenate([results['gt_bboxes'], src_boxes])
                results['gt_labels'] = np.concatenate([results['gt_labels'], src_labels])

        return results

    @staticmethod
    def computer_src_iou(src_boxes, tgt_boxes):
        src_boxes = copy.deepcopy(src_boxes)
        tgt_boxes = copy.deepcopy(tgt_boxes)
        src_boxes[:, 2] -= src_boxes[:, 0]
        src_boxes[:, 3] -= src_boxes[:, 1]
        tgt_boxes[:, 2] -= tgt_boxes[:, 0]
        tgt_boxes[:, 3] -= tgt_boxes[:, 1]
        is_crowd = [int(False)] * len(tgt_boxes)
        ious = mask_utils.iou(src_boxes, tgt_boxes, is_crowd)
        return ious.max(axis=1)


@PIPELINES.register_module()
class CopyPasteMask(object):
    ...
    def __call__(self, results):
        ...
        if random.random() < self.p:
            ...
            src_img, src_boxes, src_masks, src_labels = [src_results[k] for k in ('img', 'gt_bboxes', 'gt_masks', 'gt_labels')]
            assert results['img'].shape == src_results['img'].shape
            assert isinstance(results['gt_masks'], BitmapMasks)
            assert isinstance(src_results['gt_masks'], BitmapMasks)
            ...
            src_boxes = src_boxes[keep_indices]
            src_labels = src_labels[keep_indices]
            src_masks = src_masks[keep_indices]
            ...
            if len(src_boxes) > 0:
                ...
                gt_masks = np.concatenate([results['gt_masks'].masks, src_masks])
                gt_masks = BitmapMasks(gt_masks, results['gt_masks'].height, results['gt_masks'].width)
